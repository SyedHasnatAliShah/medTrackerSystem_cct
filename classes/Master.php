...
Class Master extends DBConnection {
	...
	function save_doctors(){
		...
		if($save){
			...
			$resp['status'] = 'success';
				if(empty($id))
					$this->settings->set_flashdata('success',"Doctor has been added successfully.");
				else
					$this->settings->set_flashdata('success',"Doctor has been updated.");
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Saving doctor failed';
			$resp['sql'] = $sql;
			$resp['error'] = $this->conn->error;
		}
		return json_encode($resp);
	...
	function delete_doctors(){
		extract($_POST);
		$delete = $this->conn->query("DELETE FROM `doctors_list` where id = '{$id}' ");
		if($delete){
			$resp['status'] = 'success';
	...
	function dt_doctors(){
		...
		$totalCount = $this->conn->query("SELECT * FROM `doctors_list`")->num_rows;
		$search_where = "";
		if(!empty($search['value'])){
			$search_where .= " name LIKE '%{$search['value']}%' ";
			$search_where .= " OR clinicname LIKE '%{$search['value']}%' ";
		...
		$columns_arr = array("unix_timestamp(created_at)",
							...
							"clinicname",
							"");
		$query = $this->conn->query("SELECT * FROM `doctors_list`   {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start} ");
		
		$recordsFilterCount = $this->conn->query("SELECT * FROM `doctors_list`  where id = '{$this->settings->userdata('id')}'  {$search_where} ")->num_rows;
	...
	function dt_medicines(){
		...
		if(!empty($search['value'])){
			...
			$search_where .= " OR date_format(`medicine_list`.created_at,'%M %d, %Y') LIKE '%{$search['value']}%' ";
			$search_where = " and ({$search_where}) ";
		}
		$columns_arr = array("unix_timestamp(`medicine_list`.created_at)",
							"unix_timestamp(`medicine_list`.created_at)",
							"name",
							"description",
							"");
		$query = $this->conn->query("SELECT medicine_list.id,medicine_list.name ,description, medicine_list.created_at, doctors_list.name AS `doctor_name`  FROM `medicine_list`  LEFT JOIN `doctors_list` on `doctors_list`.`id` = `doctor_id` where user_id = '{$this->settings->userdata('id')}'  {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start} ");
		
		$recordsFilterCount = $this->conn->query("SELECT * , `doctors_list`.`name` AS `doctor_name` FROM `medicine_list`  LEFT JOIN `doctors_list` on `doctors_list`.`id` = `doctor_id` where user_id = '{$this->settings->userdata('id')}'  {$search_where} ")->num_rows;

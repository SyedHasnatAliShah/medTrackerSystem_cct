...
<section class="py-5">
    <div class="container">
        <h2 class="fw-bolder text-center"><b><?= isset($id) ? "Edit Doctor" : "Create New Doctor" ?></b></h2>
        <hr>
        <div class="row justify-content-center">
            <div class="col-lg-8 col-md-10 col-sm-12 col-xs-12">
                <form action="" id="doctor-form" class="py-3">
                    <input type="hidden" name="id" value="<?= isset($id) ? $id : '' ?>">
                    <div id="filter-holder">
                        <div class="input-group mb-3 input-group-dynamic <?= isset($name) ? 'is-filled' : '' ?>">
                            <label for="name" class="form-label">Doctor Name <span class="text-primary">*</span></label>
                            <input type="text" id="name" name="name" value="<?= isset($name) ? $name : "" ?>" autofocus class="form-control">
                        </div>
                    </div>
					<div id="filter-holder">
                        <div class="input-group mb-3 input-group-dynamic <?= isset($clinicname) ? 'is-filled' : '' ?>">
                            <label for="clinicname" class="form-label">Clinic Name <span class="text-primary">*</span></label>
                            <input type="text" id="clinicname" name="clinicname" value="<?= isset($clinicname) ? $clinicname : "" ?>" autofocus class="form-control">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12 text-center">
                            ...
                            <a href="./?page=doctors" class="btn bg-deafult border bg-gradient btn-sm w-25"><span class="material-icons">keyboard_arrow_left</span> Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    ...
    $(function(){
        $('#doctor-form').submit(function(e){
            ...
            start_loader()
            $.ajax({
                url:'../classes/Master.php?f=save_doctors',
                method:'POST',
                data:$(this).serialize(),
                dataType:'json',
                error:err=>{
